...
terraform {
  ...
  backend "s3" {
    bucket         = "craigc-devops-unique-bucket"
...
module "bootstrap" {
  ...
  name_of_s3_bucket           = "craigc-devops-unique-bucket"
...
locals {
  ...
  my_public_ip          = "2.40.27.115/32"     # Update this to your public IP when deploying
